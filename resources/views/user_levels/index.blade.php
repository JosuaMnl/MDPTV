                    ...
                    @foreach ($user_levels as $item)
                        <tr>
                            ...
                            <td>
                                ...
                                <form method="POST" action="{{ route('user_levels.destroy', $item->id) }}"
                                    style="display: inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-danger show_confirm"
                                        data-name="{{ $item->user_levels }}" data-toggle="tooltip"
    ...
    @push('modalDelete')
        <script type="text/javascript">
            $('.show_confirm').click(function(event) {
                ...
                swal({
                        title: 'Apakah Anda yakin ingin menghapus data user level' + name + '?',
